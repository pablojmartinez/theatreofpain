...
ENT.Author = "Chessnut & Pablo J. Martínez"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if (SERVER) then
	...
	function ENT:Use(activator)
        ...
        local activatorCharacter = activator:getChar()
        local activatorCharacterName = activatorCharacter:getName()
        local activatorCharacterGroup = activatorCharacter:getData("CharacterGroup", nil)

        if(self.PadlockOwner ~= activatorCharacterName) then
            if(self.CharacterGroup == nil or self.CharacterGroup ~= activatorCharacterGroup) then
                self:error()
                return
            end
        else
            self.CharacterGroup = activatorCharacterGroup
        end
	...
	function ENT:error()
		self:EmitSound("foley/eli_fall_against_table.wav")
	...
	function ENT:getLockPos(client, door)
		...
		position = position + -normal:Forward()*1 + -normal:Up()*2
