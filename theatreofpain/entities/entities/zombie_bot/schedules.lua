...
function ENT:ResetAIVariables()
    ...
    print("MY NIGGAAAAAAAAAAAAAAAAAAAAAAAAAA")
...
function ENT:Claw()
    ...
    while(CurTime() < self.TaskSequenceEnd and self:HaveEnemy()) do
...
function ENT:Bite()
	...
	--if(targ and self.Alive) then
    if(self:GetEnemy() and self.Alive) then
		
		self.BiteTarg = self:GetEnemy();
