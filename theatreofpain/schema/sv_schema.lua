...
function SCHEMA.Z.CreateZombie(pos)
	
	local e = ents.Create( "zombie_bot" );
	e:SetPos( pos );
	e:Spawn();
	e:Activate();
...
-- By Pablo J. Martínez
--[[function SCHEMA.Z.SetZombiesHealth()
    local health = nut.config.get("ZombiesHealth")
	for k, v in pairs(SCHEMA.Z.GetZombies()) do
	    v:SetHealth(health)
    end
end--]]

--[[nut.command.add("setzombieshealth", {
	adminOnly = false,
	syntax = "[health]",
	onRun = function(client, arguments)
        local health = arguments[1]
        if (health) then
		    for k, v in pairs(SCHEMA.Z.GetZombies()) do
	            v:SetHealth(health)
                nut.config.set
