--[[Pablo J. Martínez if CLIENT then
...
function GibMod_SpawnHeadcrab( ent, damageForce, damagePos )
    ...
    local ragdollPhysicsObject = ragdoll:GetPhysicsObject() -- Pablo J. Martínez
    if(IsValid(ragdollPhysicsObject)) then -- Pablo J. Martínez
	    ragdollPhysicsObject:ApplyForceCenter( vel )
	    ragdollPhysicsObject:AddAngleVelocity( Vector(0, 0, 1000 ) ) -- backwards spin
    end
...
function GibMod_DropWeapon( ent, damageForce )
	...
	if ent:GetActiveWeapon() and ent:GetActiveWeapon():IsValid() then
        ...
        local droppedWeaponPhysicsObject = droppedWeapon:GetPhysicsObject() -- Pablo J. Martínez
        if(IsValid(droppedWeaponPhysicsObject)) then -- Pablo J. Martínez
		    droppedWeaponPhysicsObject:ApplyForceCenter( damageForce + Vector( 0, 0, 250 ) ) -- make the gun fly away
        end
	end
end
