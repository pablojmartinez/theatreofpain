	...
	if (SERVER) then
		netstream.Hook("charChoose", function(client, id)
			...
			if (character and character:getPlayer() == client) then
                ...
                -- Introduction by Pablo J. Martínez
                local firstTimeLoggedIn = character:getData("FirstTimeLoggedIn", true)
                if(firstTimeLoggedIn == true or firstTimeLoggedIn == false) then
                    netstream.Start(client, "displayAreaText", "Welcome to Theatre of Pain")
                    timer.Simple(6, function()
                        netstream.Start(client, "displayAreaText", "Developed by Pablo J. Martínez")
                        timer.Simple(6, function()
                            netstream.Start(client, "displayAreaText", "Designed by Aizer")
                            timer.Simple(6, function()
                                netstream.Start(client, "displayAreaText", "Based on Nutscript")
                                timer.Simple(6, function()
                                    netstream.Start(client, "displayAreaText", "Thanks to Disseminate, Dremek, Chessnut, Black Tea, Exho and everyone else")
                                end)
                            end)
                        end)
                    end)
                end
                character:setData("FirstTimeLoggedIn", false)
			else
